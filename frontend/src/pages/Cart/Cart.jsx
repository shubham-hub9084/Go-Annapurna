                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                    ...
                                    <div className="flex-grow">
                                        ...
                                        <div className="text-sm text-gray-500 mt-1 space-y-0.5">
                                            {item.selectedVariant && (
                                                <p><span className="font-medium text-gray-700">Size:</span> {item.selectedVariant.name}</p>
                                            )}
                                            {item.selectedAddons && item.selectedAddons.length > 0 && (
                                                <p><span className="font-medium text-gray-700">Add-ons:</span> {item.selectedAddons.map(a => a.name).join(", ")}</p>
                                            )}
                                            {item.specialRequest && (
                                                <p className="text-xs italic text-orange-600">Note: "{item.specialRequest}"</p>
                                            )}
                                        </div>
                                        <p className="text-gray-900 font-semibold text-sm mt-2">₹{item.price} / item</p>
                                    </div>
                                    ...
                                    <div className="flex flex-col items-end gap-2">
                                        ...
                                        <div className="flex items-center gap-3 bg-gray-100 rounded-lg p-1">
                                            ...
                                            <button
                                                ...
                                                className="w-8 h-8 flex items-center justify-center bg-white rounded-md shadow-sm text-gray-600 hover:text-green-500 transition-colors"
                                            >
                                                <RiAddLine />
                                            </button>
                                        </div>
                    ...
                    <div className="lg:col-span-1">
                        ...
                        >
                            ...
                            <Link to="/checkout" className="block w-full py-4 bg-gray-900 text-white rounded-xl font-bold text-lg hover:bg-black transition-all shadow-xl hover:shadow-2xl active:scale-95 text-center">
                                Proceed to Checkout
                            </Link>
                            <p className="text-center text-xs text-gray-400 mt-4">Secure Checkout Powered by Razorpay</p>
                        </motion.div>
                    </div>
                </div>
            </div>
